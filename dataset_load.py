...
class Dataset() :
    def __init__(self, name, path, min_length=None, max_length=None, args=None) :
        ...
        if args.encoder == 'bert':
            X_text, Xt_text = self.vec.raw_text['train'], self.vec.raw_text['test']
        ...
        if args.pre_loaded_attn or args.mode == "adv" or args.mode =='adv-ours':
            ...
            if args.encoder == 'bert':
                self.train_data_text = DataHolder(X_text, y)
                self.test_data_text = DataHolder(Xt_text, yt)
